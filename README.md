...
[![Version](https://img.shields.io/badge/نسخه-2.11.2-0ea5a6?style=flat-square)]()
...
## 🆕 v2.11.2 — رفع رفرش چندباره + مشخصات کامل دستگاه + ریپوی یکتا

- **رفع «چند بار رفرش شدن صفحه» هنگام باز کردن اپ (درخواست کاربر):** ریشه واقعی، دو reload پشت‌سرهم بود — یکی از خود اپ (بررسی version.json) و یکی از گارد کش مرورگر داخل صفحه. اکنون اپ دیگر reload اضافه نمی‌کند و صفحه حداکثر **یک‌بار** و بدون حلقه تازه‌سازی می‌کند.
- **مشخصات کامل گوشی برای مدیریت لایسنس (درخواست کاربر):** تپ قلب اپ حالا متادیتای جامع می‌فرستد (برند، مدل، دستگاه/محصول، نسخه اندروید + API + وصله امنیتی، رزولوشن و تراکم صفحه، زبان، رم کل، شناسه بسته، زمان نصب/بروزرسانی، منطقه زمانی) — پنل مدیریت لایسنس در دکمهٔ **«جزئیات»** هر دستگاه همه را نمایش می‌دهد.
- **دو اپ روی یک گوشی = دو دستگاه مستقل (درخواست کاربر):** اپ مدیر و اپ سرویس‌کار دیگر روی هم نمی‌افتند — هر نقش ردیف خودش را در «دستگاه‌های مدیریت لایسنس» دارد.
- **این ریپو = مرجع یکتای اپ‌های اندروید سهند سرویس (درخواست کاربر):** از v2.11.2 سورس کامل اپ‌ها فقط اینجاست (قبلاً هم در `cPanel_Sahand_SahandService` هم اینجا بود و دوگانگی می‌ساخت). پنل‌ها فقط **APK نهایی** را در `downloads/` خود برای بروزرسانی خودکار نگه می‌دارند.
- کلید امضای release (`keystore/SahandService-release.keystore`) در گیت نیست — از بستهٔ امنیتی `Sahand_Security_Package.zip` بازیابی کنید (رمزها داخل همان بسته).
- versionCode **13** — نسخهٔ **2.11.2**

## 🆕 v2.11.1 — سرویس اعلان‌های مقاوم + تپ قلب + آیکون‌های متمایز

- **اعلان‌های همیشه‌زنده:** `NotifyService` پیش‌زمینه (START_STICKY) + `BootReceiver` (بعد از ری‌استارت گوشی) + `NotificationHub` با شمارنده‌های مشترک — اعلان‌ها حتی با بسته شدن اپ نمایش داده می‌شوند و پیام‌های موجود خوانده‌نشده در اولین بررسی هم اعلان می‌شوند.
- **ثبت دستگاه در سرور لایسنس:** هر ۶ ساعت + هر ورود، تپ قلب با `ANDROID_ID` و نقش flavor به `/api/app-heartbeat` پنل → `v1/heartbeat` سرور لایسنس → نمایش در پنل مدیریت لایسنس.
- **آیکون‌های متمایز flavor:** بج آبی «ن» (نمایندگی) و بج سبز «س» (سرویس‌کار) روی آیکون اپ.
- versionCode **12** — نسخهٔ **2.11.1**
...
## v2.9.10 — تحویل تضمینی منوهای جدید (۱۴۰۵/۰۶/۱۶)

**گزارش کاربر:** «موارد جدیدی که به منو اضافه شده مثل گزارش‌گیری سفارشی و مابقی موارد جدید در اپلیکیشن وجود نداره»

- **ریشه:** کش HTTP وب‌ویو نسخهٔ کهنهٔ index.html/چانک‌ها را نگه می‌داشت — `clearCache(true)` ناهمگام است و ممکن است پیش از `reload()` تمام نشده باشد.
- **فیکس:** `reloadBypassingCache()` — `LOAD_NO_CACHE` + `clearCache(true)` + reload تأخیری (۳۰۰ms) + بازگشت به `LOAD_DEFAULT` در `onPageFinished`.
- versionCode 9 / versionName 2.9.10 / UA `SahandAndroidApp/2.9.10`
