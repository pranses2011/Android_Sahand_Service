...
## ۷) 🔒 قانون طلایی انتشار (پس از حادثهٔ پاک‌شدن 2.12.7/1.2.4 — ۲۰۲۶-۰۹-۰۶)

**چه اتفاقی افتاد؟** نسخه‌های رسمی 2.12.7 و 1.2.4 فقط داخل zipهای رلیس ساخته شدند و تغییراتشان **هرگز به گیت push نشد**. با ریست‌شدن محیط کاری، جلسهٔ بعد از کلون قدیمی (2.12.6/1.2.3) بیلد گرفت و رلیز ساخت؛ کاربر آن را نصب کرد و تغییرات پروداکشن **پاک شد**.

**قوانین اجباری از v2.12.8:**
1. **هر نسخهٔ رسمی = کامیت push‌شده.** قبل از ساخت هر بستهٔ رلیز، همهٔ تغییرات commit و push شوند؛ تگ نسخه زده شود.
2. **گارد خط پایه:** فایل `release-baseline.json` آخرین نسخهٔ رسمیِ نصب‌شده روی پروداکشن را نگه می‌دارد؛ قبل از بیلد اجرا کنید:
   `php tools/check-release.php`
   اگر نسخهٔ فعلی ≤ baseline باشد، بیلد متوقف می‌شود. پس از انتشار موفق، baseline را به نسخهٔ جدید به‌روز کنید و push کنید.
3. **قبل از شروع هر جلسهٔ کاری:** `git fetch` + مقایسهٔ HEAD با `origin/branch` + خواندن `worklog.md`. اگر ریپو از ریموت عقب است، هرگز روی وضعیت محلی قدیمی بیلد نگیرید.
4. **بستهٔ FULL رسمی** از همان کامیتِ تگ‌خورده ساخته شود (نه از working-tree بدون کامیت).
5. کلید امضای APK فقط `SahandService-release.keystore` (بستهٔ امنیتی) — هرگز کلید دیگر (آپدیت مستقیم نصب‌ها نمی‌نشیند).
